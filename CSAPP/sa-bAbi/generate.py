...
def main(args):
    ...
    generators = [gen_safe_deref_example,gen_unsafe_deref_example
                 # gen_cond_example, gen_while_example, gen_for_example,
...
def gen_safe_deref_example(include_cond_bufwrite = True):
    ...
    dereference_lines = templates.SAFE_DEREFERENCE_LINES
...
def _assemble_ptr_example(ptr_init_pairs,dereference_lines,safe,substitutions):
    lines, body_tags = _get_ptr_lines(ptr_init_pairs, dereference_lines, safe)
...
def _get_ptr_lines(ptr_init_pairs, dereference_lines, safe):
    setup_lines = _get_setup_lines(ptr_init_pairs)
    lines = setup_lines + dereference_lines
    # construct body tags before adding dummies
    body_tags = [Tag.BODY for _ in lines]

    query_tag = Tag.PTR_DEREF_SAFE if safe else Tag.PTR_DEREF_UNSAFE
